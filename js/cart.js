...
checkoutNow.onclick = ()=>{
  ...
  if(!window.PAYPAL_ME_USERNAME || window.PAYPAL_ME_USERNAME==='YOUR_PAYPAL_ME'){
    if(!confirm('PayPal.me username is not set in config. Continue to test with €5?')) return;
    // redirect to a generic paypal.me (demo)
    window.location.href = `https://www.paypal.me/${window.PAYPAL_ME_USERNAME}/5`;
    return;
  }
  const amount = total.toFixed(2);
  window.location.href = `https://www.paypal.me/${window.PAYPAL_ME_USERNAME}/${amount}`;
}

clearCart.onclick = ()=>{ if(confirm('Clear the cart?')){ localStorage.removeItem(CART_KEY); render(); }}
